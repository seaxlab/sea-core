  ...
  public static void check(PageInfo pageInfo) {
    ...
    if (pageInfo.getPageMaxSize() == null) {
      if (pageInfo.getPageSize() > MAX_PAGE_SIZE) {
        log.warn("pageSize[{}] > default[{}], so set to {}", pageInfo.getPageSize(), MAX_PAGE_SIZE, MAX_PAGE_SIZE);
        pageInfo.setPageSize(MAX_PAGE_SIZE);
      }
    } else {
      if (pageInfo.getPageSize() > pageInfo.getPageMaxSize()) {
        log.warn("pageSize[{}] > pageMaxSize[{}], so set to {}", pageInfo.getPageSize(), pageInfo.getPageMaxSize(),
          pageInfo.getPageMaxSize());
