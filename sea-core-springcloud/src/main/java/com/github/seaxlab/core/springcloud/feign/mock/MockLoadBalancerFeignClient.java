...
public class MockLoadBalancerFeignClient extends FeignBlockingLoadBalancerClient {
    ...
    public MockLoadBalancerFeignClient(Client delegate, LoadBalancerClient loadBalancerClient, LoadBalancerProperties properties, LoadBalancerClientFactory clientFactory, FeignMockProperties apiMockProperties) {
     ...
     * @return
     */
    private Response parseProtocol(Request request, Request.Options options, String mockApi) throws IOException {
        ...
        switch (protocol.trim()) {
            case "local":
                ...
                if (FileUtil.exist(path)) {
                    ...
                    return Response.builder().headers(headers).status(HttpStatus.OK.value()).request(request).body(FileUtil.readFileToBytes(path)).build();
                } else {
                    ExceptionHandler.publishMsg("feign mock local file is not exist [{}], so invoke default.", path);
                }
            case "remote":
                log.info("====");
                ...
                Request newRequest = Request.create(Request.HttpMethod.GET, realUri.trim(), Collections.emptyMap(), null, Util.UTF_8, null);
