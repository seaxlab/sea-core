...
public final class BatchUtil {
  ...
  /**
   * 批量添加，必须开启事务<br/> 重点：这段代码中开启了session，所以外层中必须开启事务
   *
  ...
  /**
   * 批量插入，必须开启事务<br/> 重点：这段代码中开启了session，所以外层中必须开启事务
   *
  ...
  /**
   * 批量更新，必须开启事务<br/> 重点：这段代码中开启了session，所以外层中必须开启事务
   *
   * @param sqlSessionFactory
   * @param mapperClass
   * @param list
   * @param <T>
   */
  public static <T extends UpdateByPrimaryKeySelectiveMapper> void updateByPrimaryKeySelective(
