...
import com.github.seaxlab.core.component.template.cache.IListQueryCache;
 ...
 */
@Slf4j
public abstract class BaseListQueryCache<V> implements IListQueryCache<V> {
  ...
  // 缓存实例
  private volatile LoadingCache<String, List<V>> cache;
  private static final String BIZ_KEY = "list_cache_biz_key";
   ...
   */
  protected abstract List<V> fetchData();
   ...
   */
  private LoadingCache<String, List<V>> getCache() {
  ...
  private void buildCache() {
    ...
    cache = builder.build(new CacheLoader<String, List<V>>() {
      @Override
      public List<V> load(String key) {
        log.info("try to load cache[{}]", getBizType());
        List<V> value = fetchData();
        return Objects.isNull(value) ? ListUtil.empty() : value;
      }
    });
