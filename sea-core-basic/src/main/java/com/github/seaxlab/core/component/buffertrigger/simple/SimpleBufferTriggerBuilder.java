...
import com.github.seaxlab.core.component.buffertrigger.common.BackPressureHandler;
import com.github.seaxlab.core.component.buffertrigger.common.BackPressureListener;
import com.github.seaxlab.core.component.buffertrigger.common.LazyBufferTrigger;
import com.github.seaxlab.core.component.buffertrigger.common.RejectHandler;
import com.github.seaxlab.core.component.buffertrigger.simple.strategy.MultiIntervalTriggerStrategy;
...
public class SimpleBufferTriggerBuilder<E, C> {
   ...
   * 一般该函数调用位于处理请求线程池，该函数的耗时将直接影响业务服务并发效率， 推荐使用者实现更贴合实际业务的高性能线程安全容器.
   * <p>
   ...
   * <p>
   * 为了保证不同场景下的最优性能，框架本身不保证{@link BufferTrigger#enqueue(Object)}操作的线程安全，
   * 一般函数调用位于处理请求线程池，该函数的耗时将直接影响业务服务并发效率， 推荐使用者实现更贴合实际业务的线程安全容器.
   * <p>
   * 另，如确定{@link BufferTrigger#enqueue(Object)}为单线程调用，无需使用线程安全容器.
   *
   * @param factory    缓存提供器；要求为{@link Supplier}，因为执行消费回调后，缓存会被清空，需要执行提供器再次获得可用缓存对象
   ...
   * @param <C1>       缓存容器类型
   */
  public <E1, C1> SimpleBufferTriggerBuilder<E1, C1> setContainerEx(Supplier<? extends C1> factory,
   ...
   * 设置消费触发策略.
   *
   * @param triggerStrategy {@link TriggerStrategy}具体实现， 推荐传入{@link MultiIntervalTriggerStrategy}实例
   */
  ...
  public SimpleBufferTriggerBuilder<E, C> interval(LongSupplier interval, TimeUnit unit) {
    this.triggerStrategy = (last, change) -> {
      ...
      return TriggerResult.trig(change > 0 && currentTimeMillis() - last >= intervalInMs,
        intervalInMs);
    };
    return this;
  }
   ...
   * 设置消费回调函数.
   * <p>
   * 该方法用于设定消费行为，由使用者自行提供消费回调函数，需要注意， 回调注入的对象为当前消费时间点，缓存容器中尚存的所有元素，非逐个元素消费.
   *
   * @param consumer
   * @param <E1>     缓存元素类型
   * @param <C1>     缓存容器类型
   ...
   */
  public <E1, C1> SimpleBufferTriggerBuilder<E1, C1> enableBackPressure(
    BackPressureListener<E1> listener) {
    ...
    if (rejectHandler instanceof BackPressureHandler) {
      if (disableSwitchLock) {
        throw new IllegalStateException(
          "back-pressure cannot work together with switch lock disabled.");
      }
  ...
  private void ensure() {
    if (triggerStrategy == null) {
      ...
      triggerStrategy = (t, n) -> TriggerResult.empty();
    }
  ...
  private ScheduledExecutorService makeScheduleExecutor() {
    String threadPattern = name == null ? "pool-simple-buffer-trigger-thread-%d"
      : "pool-simple-buffer-trigger-thread-[" + name + "]";
    return newSingleThreadScheduledExecutor(
      new ThreadFactoryBuilder().setNameFormat(threadPattern).setDaemon(true).build());
  }
