...
public final class BatchConsumerTriggerBuilder<E> {
   ...
   * 设置消费回调函数.
   * <p>
   * 该方法用于设定消费行为，由使用者自行提供消费回调函数，需要注意， 回调注入的对象为缓存队列中尚存的所有元素，非逐个元素消费.
   */
  public <E1> BatchConsumerTriggerBuilder<E1> setConsumerEx(
    ThrowableConsumer<? super List<E1>, Exception> consumer) {
  ...
  private ScheduledExecutorService makeScheduleExecutor() {
    ...
    return newScheduledThreadPool(1,
      new ThreadFactoryBuilder().setNameFormat("pool-batch-consume-blocking-queue-thread-%d")
