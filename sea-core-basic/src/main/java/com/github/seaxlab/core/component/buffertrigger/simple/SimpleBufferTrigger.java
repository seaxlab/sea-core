...
import com.github.seaxlab.core.component.buffertrigger.common.GlobalBackPressureListener;
 ...
 * <p>
 * 消费触发策略会考虑消费回调函数的执行时间，实际执行间隔 = 理论执行间隔 - 消费回调函数执行时间； 如回调函数执行时间已超过理论执行间隔，将立即执行下一次消费任务.
 *
 * @author w.vela
 */
public class SimpleBufferTrigger<E, C> implements BufferTrigger<E> {
  ...
  public static SimpleBufferTriggerBuilder<Object, Map<Object, Integer>> newCounterBuilder() {
    return new SimpleBufferTriggerBuilder<Object, Map<Object, Integer>>().setContainer(
      ConcurrentHashMap::new, (map, element) -> {
   ...
   *
   * <p>存在缓存容量检测，如果缓存已满，
   * 且拒绝回调方法已通过{@link SimpleBufferTriggerBuilder#rejectHandler(Consumer)}设置， 则会回调该方法.
   *
   * <p>需要特别注意，该实现为了避免加锁引起的性能损耗，以及考虑到应用场景，缓存容量检测不进行严格计数，
   * 在并发场景下缓存中实际元素数量会略微超过预设的最大缓存容量； 如需严格计数场景，请自行实现{@link BufferTrigger}.
   *
   ...
   * <p>
   * 如未通过{@link
   * GenericSimpleBufferTriggerBuilder#setScheduleExecutorService(ScheduledExecutorService)}
   * 自定义计划任务线程池，会调用默认计划任务线程池shutdown()方法，如未停止成功， 会保留1天的超时时间再强制停止线程池.
   */
  @Override
  public void close() {
    shutdown = true;
    ...
    public void run() {
      synchronized (SimpleBufferTrigger.this) {
        ...
        try {
          ...
          if (triggerResult.isConsumerFlag()) {
