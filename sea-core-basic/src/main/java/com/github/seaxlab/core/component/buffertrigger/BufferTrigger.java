...
import com.github.seaxlab.core.component.buffertrigger.batch.BatchConsumerTriggerBuilder;
import com.github.seaxlab.core.component.buffertrigger.batch.GenericBatchConsumerTriggerBuilder;
import com.github.seaxlab.core.component.buffertrigger.simple.GenericSimpleBufferTriggerBuilder;
...
public interface BufferTrigger<E> extends AutoCloseable {
   ...
   *
   * @param <E> 元素类型
   * @param <C> 持有元素的容器类型，如使用默认容器实现（不指定
   *            {@link GenericSimpleBufferTriggerBuilder#setContainerEx}），默认为
  ...
  static <E> GenericBatchConsumerTriggerBuilder<E> batchBlocking() {
    return new GenericBatchConsumerTriggerBuilder<>(new BatchConsumerTriggerBuilder<>());
  }
