...
import com.github.spy.sea.core.thread.NamedThreadFactory;
...
/**
 * actor模型消息事件处理，类似EventBus
 * <p>
 * actor,proactor,reactor之间的差异//TODO
 * <p>
 * 《A Universal Modular Actor Formalism for Artificial Intelligence 》
 * </p>
 * <p>
 * 在Actor模型中，actor是一个并发原语，简单的说，一个actor就是一个工人，与进程或线程一样都能够工作或处理任务。
 * 其实这还有点不好理解，我们可以把它想象成面向对象编程语言中的一个对象实例。在OOP中一个对象可以访问或修改另一个对象的属性，
 * 也可以直接调用另一个对象的方法。例如下图，person1给person2发送了一个消息，直接调用方法就行了
 * </p>
 * <p>
 * actor和对象的不同之处在于，actor的状态不能直接读取、修改，actor的方法不能直接调用。actor只能通过消息传递的方式与外界通信。
 * 每个对象都有一个this指针，代表对象的地址，可以通过该地址调用方法或存取状态；与此类似，actor也有一个代表本身的地址，但只能向该地址发送消息。
 * <p>
 * 　　简单点说，actor通过消息传递的方式与外界通信。消息传递是异步的。每个actor都有一个邮箱，该邮箱接收并缓存其他actor发过来的消息，
