package com.github.spy.sea.core.lang.map;
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    public ConcurrentReferenceHashMap(
        ...
        Precondition.checkTrue(initialCapacity >= 0, "Initial capacity must not be negative");
        Precondition.checkTrue(loadFactor > 0f, "Load factor must be positive");
        Precondition.checkTrue(concurrencyLevel > 0, "Concurrency level must be positive");
        Precondition.checkNotNull(referenceType, "Reference type must not be null");
    ...
    private V put(final K key, final V value, final boolean overwriteExisting) {
        return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
            ...
            protected V execute(Reference<K, V> ref, Entry<K, V> entry, Entries<V> entries) {
                ...
                Precondition.checkState(entries != null, "No entries segment");
                entries.add(value);
                return null;
            }
        });
    ...
    private class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        @Override
        public void remove() {
            Precondition.checkState(this.last != null, "No element to remove");
