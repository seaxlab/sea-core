...
import com.github.spy.sea.core.lang.cache.SoftCache;
...
public class AnnotationUtil {
    ...
    private static Map<String, Object> getMemberValues(final Annotation annotation) {
        try {
    ...
    //public static Class<? extends Annotation> getRepeatItem(final Class<? extends Annotation> annotationType) {
    ...
    public static Class<? extends Annotation> getRepeatable(final Class<? extends Annotation> annotationType) {
        ...
        return annotationMap
                ...
                .collect(
                        Collectors.toMap(
                                ...
                                e -> e.getValue()
                                      .stream()
                                      .map(i -> AnnotationUtil.annotationForMap(e.getKey(), i))
    ...
    public static Map<Class<? extends Annotation>, List<Annotation>> mergeAnnotation(
        ...
        annotationMap.put(
                annotationType,
                Collections.singletonList(mergeAnnotationValue(annotation, overwriteMap))
        );
        walkAnnotation(annotationType, annotationMap, overwriteMap);
        return annotationForMergeAnnotation(annotationMap);
    }

    public static Map<Class<? extends Annotation>, List<Annotation>> mergeAnnotation(final AnnotatedElement element) {
    ...
    private static void walkAnnotation(
        ...
        for (Annotation annotation : element.getAnnotations()) {
            ...
            // 处理当前注解
            final List<Map<String, Object>> annotations = annotationMap.getOrDefault(annotationType, new ArrayList<>());
            for (Annotation item : element.getAnnotationsByType(annotationType)) {
            ...
            //);
            //if (repeatItem != null) {
            //    final List<Map<String, Object>> itemList = annotationMap.getOrDefault(repeatItem, new ArrayList<>());
            //    for (final Annotation item : (Annotation[]) ReflectUtil.invoke(annotation, "value")) {
